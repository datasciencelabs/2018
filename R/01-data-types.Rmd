...
In this dataset each state is considered an obsrvation and five variables are reported for each state.
...
**Tip**: R comes with a very nice auto-complete functionality that saves us the trouble of typing out all the names. Try typing `murders$p` then hitting the _tab_ key on your keyboard. RStudio has many useful auto-complete features options.
...
Here the `==` is a relational operator asking if 3 is equal to 2. Remember that in R, if you just use one `=` you actually assign. You can see the other _relational operators_ by typing
...
**Advanced**: Mathematically, the values in `pop` are integers and there is an integer class in R. However, by default, numbers are assigned class numeric even when they are round integers, for example `class(1)` returns numeric. You can turn them into class integer with `as.integer(1)` or by adding an `L` like this: `1L`. Note the class by typing: `class(1L)`.
