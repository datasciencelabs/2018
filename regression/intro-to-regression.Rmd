...
Correlation and the regression line are widely used summary statistics but are often misused or misinterpreted. Anscombe's examples provide toy examples of datasets in which summarizing with correlation would be a mistake. But there are many more real life examples.
...
When we fix $X=x$ in this way we then refer to the resulting distribution of the $Y$s in the strata as the _conditional distribution_ of $Y$ given $X=x$. We write this using notation like this:
